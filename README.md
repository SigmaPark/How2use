...
## H2U_ASSERT
"H2U_ASSERT" checks an boolean expression is true.  
```cpp
int result = 1 + 1;  
...
## h2u::Are_All_True / h2u::Are_N_True
"h2u::Are_All_True" checks whether all elements in a range on 1st parameter satisfy a condition function given as 2nd parameter. "h2u::Are_N_True" do the same thing on partial range from an iterator .  
```cpp
{  
...
## h2u::Are_All_Equivalent_to / h2u::Are_N_Equivalent_to
"h2u::Are_All_Equivalent_to" checks whether all elements in a range on 1st parameter are equivalent to 2nd parameter. You can specify the function that determines whether they are equivalent on 3rd parameter ( operator== as default ) . "h2u::Are_N_Equivalent_to" do the same thing on partial range from an iterator .   
```cpp
{  
...
"h2u::Are_Equivalent_Ranges" checks whether two ranges given as 1st and 2nd parameters are the same (the number of elements are same and have the same elements in the same order) . You can specify the function that determines whether the elements are same each others ( operator== as default ) .   
```cpp
{  
	std::initializer_list<int>   
		range0{2, 4, 6, 8},   
...
Another way to apply HTML tags is to use "h2u::HTML_tag" function.  
```cpp
h2u::mdo   
...
```
 You can find "guide/[doc]\_Guide_How2use.md" files and see more powerful functions!
